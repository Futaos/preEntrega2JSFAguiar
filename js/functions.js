...
function inactiveCharacterList() {
  ...
  let searchContent = contentMaker(
...
function searchByPlayerName() {
  ...
  if (characters.some((el) => el.playerName === entryPlayerName)) {
    let charByPlayer = characters.filter(
      (el) => el.playerName === entryPlayerName
    );
    let searchContent = contentMaker(
      `PERSONAJES DE ${entryPlayerName}\n\n`,
      charByPlayer
    );
...
function searchByClass() {
  ...
  if (characters.some((el) => el.charClass === entryClass)) {
    ...
    let searchContent = contentMaker(
      `PERSONAJES DE CLASE ${entryClass}\n\n`,
      charByClass
    );
...
function giveGoldToPlayer() {
  ...
  if (characters.some((el) => el.charName === charNameToGiveGold)) {
    ...
    charIndex = characters.findIndex(
      (obj) => obj.charName === charNameToGiveGold
    );
...
function giveLevel() {
  ...
  if (characters.some((el) => el.charName === charNameToGiveLevel)) {
    ...
    charIndex = characters.findIndex(
      (obj) => obj.charName === charNameToGiveLevel
    );
    characters[charIndex].level += levelQty;

    alert(
      `Has agregado ${levelQty} niveles al personaje ${charNameToGiveLevel}.`
    );
  } else {
    alert("No existe un personaje con ese nombre.");
  }
